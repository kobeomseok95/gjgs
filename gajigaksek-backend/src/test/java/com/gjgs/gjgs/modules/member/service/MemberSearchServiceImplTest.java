...
class MemberSearchServiceImplTest {
    ...
    void search_member() throws Exception{
        ...
        PageImpl<MemberSearchResponse> dtos = new PageImpl<>(MemberDummy.createMemberSearchDtoList(25),
                pageRequest,
                25);

        when(memberQueryRepository.findPagingMemberByCondition(any(),any())).thenReturn(dtos);

        //when
        Page<MemberSearchResponse> memberSearchDtos = memberSearchService.searchMember(pageRequest, condition);

        //then
        assertEquals(dtos,memberSearchDtos);
    }
}
