...
import com.gjgs.gjgs.modules.member.exception.FormErrorCodes;
import com.gjgs.gjgs.modules.member.exception.FormException;
...
public class SignUpRequestValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (errors.hasErrors()) {
            throw new FormException(FormErrorCodes.INVALID_FORMAT, errors);
        }
        ...
        if(!zoneRepository.existsById(signUpRequest.getZoneId())){
            errors.rejectValue("zoneId","invalid.zoneId","해당 zoneId는 존재하지 않는 zoneId 입니다.");
        }

        if(categoryRepository.countCategoryByIdList(signUpRequest.getCategoryIdList()) != signUpRequest.getCategoryIdList().size()){
            errors.rejectValue("categoryIdList","invalid.categoryIdList","categoryIdList 중 존재하지 않는 categoryId가 있습니다.");
        }

        if (errors.hasErrors()) {
            throw new FormException(FormErrorCodes.INVALID_SIGNUPFORM, errors);
        }
