...
public class TeamCrudServiceImpl implements TeamCrudService {
    ...
    public CreateTeamResponse createTeam(CreateTeamRequest createTeamRequest) {
        Long memberId = memberRepository.findIdByUsername(getUsername()).orElseThrow(() -> new MemberException(MemberErrorCodes.MEMBER_NOT_FOUND));
        Team team = Team.of(createTeamRequest, Member.of(memberId));
    ...
    private void putZone(Long zoneId, Team team) {
        if (team.isNewTeamOrZoneIsDifferent(zoneId)) {
            Zone zone = zoneRepository.findById(zoneId).orElseThrow(() -> new ZoneException(ZoneErrorCodes.ZONE_NOT_FOUND));
            team.putZone(zone);
        }
    }
