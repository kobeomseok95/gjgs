...
public class TeamLeaderAuthorityCheckableImpl implements TeamLeaderAuthorityCheckable {
    ...
    public Team findTeamMembers(Long teamId) {
        return teamQueryRepository.findWithTeamMembers(teamId).orElseThrow(() -> new TeamNotFoundException(TeamErrorCodes.TEAM_NOT_FOUND));
    }

    @CheckLeader
    @Override
    public Team findZone(Long teamId) {
        return teamQueryRepository.findWithLeaderZoneCategories(teamId).orElseThrow(() -> new TeamNotFoundException(TeamErrorCodes.TEAM_NOT_FOUND));
    }

    @CheckLeader
    @Override
    public Team findLeader(Long deleteTeamId) {
        return teamQueryRepository.findWithLeader(deleteTeamId).orElseThrow(() -> new TeamNotFoundException(TeamErrorCodes.TEAM_NOT_FOUND));
    }
}
