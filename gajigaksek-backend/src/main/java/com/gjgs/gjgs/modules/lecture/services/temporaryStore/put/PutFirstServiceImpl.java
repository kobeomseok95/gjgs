...
import com.gjgs.gjgs.modules.category.exceptions.CategoryErrorCodes;
import com.gjgs.gjgs.modules.category.exceptions.CategoryException;
...
public class PutFirstServiceImpl extends AbstractFileCheck implements PutLectureService {
    ...
    protected void checkFileSize(List<MultipartFile> files, int targetSize) {
        if (files.size() != targetSize) {
            throw new LectureException(LectureErrorCodes.THUMBNAIL_IS_NOT_ONE);
        }
    }
    ...
    private CreateLectureProcessResponse saveLecture(CreateLecture.FirstRequest request, List<MultipartFile> files) throws IOException {
        ...
        Member director = currentMemberUtil.getCurrentMemberOrThrow();
    ...
    private Category getCategory(Long categoryId) {
        return categoryRepository.findById(categoryId).orElseThrow(() -> new CategoryException(CategoryErrorCodes.CATEGORY_NOT_FOUND));
    }

    private Zone getZone(Long zoneId) {
        return zoneRepository.findById(zoneId).orElseThrow(() -> new ZoneException(ZoneErrorCodes.ZONE_NOT_FOUND));
    }
