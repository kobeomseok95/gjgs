...
import static com.gjgs.gjgs.modules.payment.entity.OrderStatus.*;
...
public class Order extends BaseEntity {
    ...
    private void checkCancel() {
        if (this.getOrderStatus().equals(CANCEL)) {
            throw new OrderException(OrderErrorCodes.ORDER_IS_CANCEL);
        }
    }
    ...
    private void checkOverThreeDays(LocalDateTime startDateTime) {
        if (ChronoUnit.HOURS.between(now(), startDateTime) < 72) {
            throw new OrderException(OrderErrorCodes.ORDER_NOT_CANCEL);
        }
    }
