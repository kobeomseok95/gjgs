...
import static com.gjgs.gjgs.modules.payment.entity.OrderStatus.COMPLETE;
import static com.gjgs.gjgs.modules.payment.entity.OrderStatus.WAIT;
...
public class OrderServiceImpl implements OrderService {
    ...
    public TeamMemberPaymentResponse getTeamMemberPayment(Long scheduleId) {
        ...
        return orderQueryRepository.findTeamMemberPaymentByScheduleIdUsername(scheduleId, username).orElseThrow(() -> new OrderException(OrderErrorCodes.ORDER_NOT_FOUND));
    }
    ...
    public Long paymentTeamMember(Member member, Long scheduleId, PaymentRequest paymentRequest) {
        Order order = orderQueryRepository.findByLectureScheduleIdUsername(paymentRequest.getLectureId(), scheduleId, member.getUsername())
                .orElseThrow(() -> new OrderException(OrderErrorCodes.ORDER_NOT_FOUND));
    ...
    public OrderIdDto verifyAndCompletePayment(PaymentVerifyRequest request) throws IamportResponseException, IOException {
        ...
        Order order = orderQueryRepository.findWithCouponRewardScheduleTeamById(request.getOrderId()).orElseThrow(() -> new OrderException(OrderErrorCodes.ORDER_NOT_FOUND));
        order.setIamportUid(iamportUid);

        if(!order.equalsPaymentPrice(getPaymentAmount(iamportUid))) {
            refund(currentMemberUtil.getCurrentMemberOrThrow(), order, verifyAndGetRefund(order.getIamportUid()));
    ...
    private TeamOrder findTeamOrderOrElseThrow(Order order) {
        ...
        throw new OrderException(OrderErrorCodes.ORDER_NOT_FOUND);
    }

    @Override
    public void cancel(Member refundMember, Long orderId) throws IamportResponseException, IOException {
        Order order = orderQueryRepository.findWithCouponRewardScheduleTeamById(orderId).orElseThrow(() -> new OrderException(OrderErrorCodes.ORDER_NOT_FOUND));
