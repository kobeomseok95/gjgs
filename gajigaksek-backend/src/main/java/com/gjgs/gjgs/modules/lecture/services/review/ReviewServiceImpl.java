...
import com.gjgs.gjgs.modules.lecture.exceptions.*;
...
public class ReviewServiceImpl implements ReviewService {
    ...
    private Member validReviewerInParticipants(CreateReviewRequest request) {
        Long memberId = memberRepository.findIdByUsername(getCurrentUsername()).orElseThrow(() -> new MemberException(MemberErrorCodes.MEMBER_NOT_FOUND));
        if (!participantQueryRepository.existParticipantByScheduleIdMemberId(request.getScheduleId(), memberId)) {
            throw new ScheduleException(ScheduleErrorCodes.NOT_PARTICIPANT);
        }
        return Member.of(memberId);
    }
    ...
    public void replyReview(Long reviewId, CreateReviewReplyRequest request) {
        Review review = reviewQueryRepository.findWithLectureByReviewIdDirectorUsername(reviewId, securityUtil.getCurrentUsername().get()).orElseThrow(() -> new ReviewException(ReviewErrorCodes.NOT_EXIST_REPLY_REVIEW));
