...
import com.gjgs.gjgs.modules.team.exceptions.*;
...
public class Team extends BaseEntity implements CheckTeamLeader {
    ...
    private void checkLowerThanCurrentPeople(int maxPeople) {
        if (this.currentMemberCount > maxPeople) {
            throw new TeamCurrentPeopleHigherException(TeamErrorCodes.TEAM_CURRENT_PEOPLE_HIGHER);
        }
    }

    public void checkTeamIsFullDoThrow() {
        if (isFull()) {
            throw new TeamMembersMaxException(TeamErrorCodes.TEAM_MEMBER_IS_MAX);
        }
    }
    ...
    public void addTeamMemberCount() {
        ...
        if (currentMemberCount > maxPeople) {
            throw new TeamException(TeamErrorCodes.TEAM_MEMBER_IS_MAX);
        }
    }
}
